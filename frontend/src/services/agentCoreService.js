 ...
 *
 * @param {Function} onStatusUpdate - Optional callback for status events
 */
export const invokeAgentCore = async (query, sessionId, onStreamUpdate, accessToken, userId, onStatusUpdate = null) => {
  try {
    if (!userId) {
    ...
    // Set up headers
    // Note: X-Amzn-Bedrock-AgentCore-Runtime-User-Id is required for 3-Legged OAuth (Authorization Code Grant)
    const headers = {
      Authorization: `Bearer ${accessToken}`,
      "X-Amzn-Trace-Id": traceId,
      "Content-Type": "application/json",
      "X-Amzn-Bedrock-AgentCore-Runtime-Session-Id": sessionId,
      "X-Amzn-Bedrock-AgentCore-Runtime-User-Id": userId,  // Required for OAuth on behalf of users
    }
      ...
      try {
        while (true) {
          ...
          for (const line of lines) {
            if (line.trim()) {
              completion = currentParser.parseStreamingChunk(line, completion, onStreamUpdate, onStatusUpdate);
            }
          }
        }
      } finally {
