...
export class CognitoStack extends cdk.NestedStack {
  ...
  constructor(scope: Construct, id: string, props: CognitoStackProps) {
    const description = "Healthcare Agents Authentication - Cognito User Pool with OAuth 2.0, custom domain, and machine-to-machine client for agent access";
  ...
  private createCognitoUserPool(config: AppConfig, callbackUrls?: string[]): void {
    ...
    const userPool = new cognito.UserPool(this, "UserPool", {
      ...
      // Threat T1 — strengthen password policy to NIST 800-63B
      // baseline + require symbols. 12 chars + complexity to slow brute force.
      passwordPolicy: {
        minLength: 12,
      ...
      // Threat T1 — require MFA for every provider account so
      // stolen credentials alone are not sufficient for impersonation.
      // OPTIONAL allows TOTP enrollment via the managed login UI; switch to
      // REQUIRED once all real users have completed enrollment.
      mfa: cognito.Mfa.REQUIRED,
      mfaSecondFactor: {
        sms: false,           // SMS is HIPAA-questionable + SIM-swap risk
        otp: true,             // TOTP authenticator app
      },
      // Threat T1 — adaptive auth blocks suspicious sign-ins
      // Note: advancedSecurityMode removed as it requires PLUS pricing tier.
      // Use standardThreatProtectionMode for ESSENTIALS tier if needed.
