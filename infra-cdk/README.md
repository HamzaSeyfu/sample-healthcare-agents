...
### Single Pattern Deployment (Legacy)

```yaml
stack_name_base: healthcare-agents-stack

backend:
  pattern: medical-coding-agent  # Deploy one pattern
  deployment_type: docker
```

### Multi-Pattern Deployment (Recommended)

```yaml
stack_name_base: healthcare-agents-stack

backend:
  # Deploy multiple agent patterns simultaneously
  patterns:
...
**📚 See [MULTI_PATTERN_DEPLOYMENT.md](./MULTI_PATTERN_DEPLOYMENT.md) for complete documentation**

### Available Patterns

- `prior-authorization-agent` - Prior authorization assessment with FHIR Claim bundles
- `eligibility-verification-agent` - Insurance eligibility and benefit verification
- `medical-coding-agent` - Medical coding and billing agent with Knowledge Base
...
- `appeals-agent` - Claim denial analysis and appeal letter generation
