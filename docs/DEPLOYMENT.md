...
FAST supports optional observability features via environment variables. Create a `.env` file in the project root:
...
Langfuse provides tracing and observability for agent invocations. This is **optional** - deployment will work without it.

**Setup Options:**

1. **Use shared team Langfuse instance** (contact team lead for credentials)
2. **Create your own free instance** at https://cloud.langfuse.com
...
**Security:** For HIPAA-compliant healthcare deployments, credentials are automatically stored in AWS Secrets Manager during CDK deployment. The agent fetches credentials at runtime from Secrets Manager instead of using Lambda environment variables. This prevents credential exposure and supports secure rotation.

**Note:** CDK will automatically skip Langfuse configuration if these variables are not present. This allows you to deploy and test without observability features. An empty Secrets Manager secret will be created that can be populated later via:

```bash
aws secretsmanager update-secret \
  --secret-id /<your-stack-name>/langfuse/credentials \
  --secret-string '{"publicKey":"pk-lf-...","secretKey":"sk-lf-...","baseUrl":"https://cloud.langfuse.com"}'
```
